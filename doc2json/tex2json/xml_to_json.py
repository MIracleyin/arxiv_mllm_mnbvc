...
# def read_image(image_path, dpi=300):
#     # 打开图像文件
#     if image_path.endswith('pdf'):
#         image = convert_from_path(image_path, dpi)[0]
#     else:
...
def get_figure_map_from_tex(sp: BeautifulSoup, latex_dir: str) -> Dict:
    """
    ...
    for flt in sp.find_all('float'):
        try:
            if flt.name and flt.get('name') == 'figure':
                ...
                with open(fig_name, 'wb') as file:
                    file.write(decoded_bytes)
                '''
                for fig in flt.find_all('figure'):
                    if fig.get('file') and fig.get('extension'):
                        fname = fig.get('file') + '.' + fig.get('extension')
                        figure_path = os.path.join(latex_dir, fname)

                        # with open(figure_path, 'rb') as figure:
                        #     base64_str = base64.b64encode(figure.read()).decode('utf-8')
                        # img_binary, img_size = read_image(figure_path)
                        # fig_contents.append(img_binary)
                        fig_files.append(figure_path)
                    elif fig.get('file'):
                        fname = fig.get('file')
                        figure_path = os.path.join(latex_dir, fname)
                        # with open(figure_path, 'rb') as figure:
                        #     base64_str = base64.b64encode(figure.read()).decode('utf-8')
                        #     fig_contents.append(base64_str)
                        # img_binary, img_size = read_image(figure_path)
                        # fig_contents.append(img_binary)
                        fig_files.append(figure_path)
                    else:
                        for subfig in fig.find_all('subfigure'):
                            if subfig.get('file') and subfig.get('extension'):
                                figure_path = os.path.join(latex_dir, subfig.get('file') + '.' + subfig.get('extension'))
                                fig_files.append(figure_path)
                                ...
                                figure_path = os.path.join(latex_dir, subfig.get('file'))
                ...
                if flt.get('id'):
                    ...
                    figure_map[ref_id] = {
                        ...
                        "uris": fig_files, # 在这里增加pic的base64表示
                        "image_binary": fig_contents,
                        "ref_id": ref_id
                    }
        except AttributeError:
        ...
        try:
            if fig.name and fig.get('id'):
                ...
                if fig.get('file') and fig.get('extension'):
                    ...
                    figure_path = os.path.join(latex_dir, fig.get('file'))
    ...
    for i, row in enumerate(table_lst):
        ...
        if i == 0 or bottom_border:
            for cell in row['cells']:
                html_str += f"<th>{cell['latex']}</th>"
                # html_str += f"<th>{cell['text']}</th>"
        else:
            for cell in row['cells']:
                # html_str += f"<td>{cell['text']}</td>"
                html_str += f"<td>{cell['latex']}</td>"
        html_str += '</tr>'
    html_str += '</table>'
    return html_str
...
def extract_table_html(table: BeautifulSoup) -> List:
    """
    Extract table values from table entry
    :param table:
    :return:
    ...
    for row in table.find_all('tr'):
        cells = []
        cols = [cell for cell in row.find_all(['th', 'td'])]
        for col in cols:
            text_items = []
            latex_items = []
            for cell in col:
            ...
            latex = re.sub(r'\s+', ' ', latex)
            alignment = 'center'
            if sv.match('.ltx_align_left', col):
                alignment = 'left'
            elif sv.match('.ltx_align_right', col):
                alignment = 'right'
            elif sv.match('.ltx_align_justify', col):
                alignment = 'justify'

            cells.append({
                "alignment": alignment,
                "right-border": sv.match('.ltx_border_r, .ltx_border_rr, .ltx_border_R', col),
                "left-border": sv.match('.ltx_border_l, .ltx_border_ll, .ltx_border_L', col),
                "text": text.strip(),
                "latex": latex.strip()
            })
        table_rep.append({
            "top-border": sv.match('.ltx_border_t, .ltx_border_tt, .ltx_border_T', row),
            "bottom-border": sv.match('.ltx_border_b, .ltx_border_bb, .ltx_border_B', row),
            "cells": cells
        })
    return table_rep
...
def normalize_html(sp: BeautifulSoup) -> BeautifulSoup:
    ...
    for row in sp.find_all('tr'):
        for col in row.find_all(['th', 'td']):
            ...
            if math_tag:
                alttext_value = math_tag.get('alttext', '')
                col.clear()
                col.string = alttext_value
            else:
                child_tags = col.find_all(True)  # True 查找所有标签
        ...
        try:
            if flt.name and flt.get('name') == 'table':
                if flt.get('id'):
                    ...
                    table_map[flt.get('id-text', None)] = ref_id
        except AttributeError:
    ...
    for tab in sp.find_all('table'):
        try:
            ...
            if tab.name and tab.get('id'):
                ...
                table_map[tab.get('id-text', None)] = ref_id
        except AttributeError:
...
def get_table_list_from_tex(tex_file, table_map):
    tex_code = open(tex_file, 'r', encoding='utf-8').read()
    table_lists = []
...
def convert_xml_to_s2orc(
        xml_sp: BeautifulSoup, html_sp: BeautifulSoup, file_id: str, year_str: str, log_file: str, temp_dir: str, grobid_config: Optional[Dict]=None
) -> Paper:
    """
    ...
    decompose_tags_before_title(xml_sp)
    decompose_tags_before_title(xml_sp)

    # process maketitle info
    title, authors = process_metadata(xml_sp, client, log_file)

    # processing of bibliography entries
    # TODO: look into why authors aren't processing
    bibkey_map = process_bibliography_from_tex(xml_sp, client, log_file)
    ...
    section_map = process_sections_from_text(xml_sp)

    # process and replace non-inline equations
    equation_map = process_equations_from_tex(xml_sp)

    # process footnote markers
    footnote_map = process_footnotes_from_text(xml_sp)

    # get figure map
    latex_dir = os.path.join(temp_dir, 'latex', file_id)
    ...
    refkey_map = process_figures_from_tex(xml_sp, refkey_map)

    # process and replace tables
    # refkey_map = process_tables_from_tex(xml_sp, refkey_map)

    # collapse all hi tags
    collapse_formatting_tags(xml_sp)

    # process abstract if possible
    abstract = process_abstract_from_tex(xml_sp, bibkey_map, refkey_map)

    # process body text
    body_text = process_body_text_from_tex(xml_sp, bibkey_map, refkey_map)
...
def convert_latex_xml_to_s2orc_json(xml_fpath: str, html_fpath: str, temp_dir: str, log_dir: str, grobid_config: Optional[Dict]=None) -> Paper:
    """
    :param xml_fpath:
    :param log_dir:
    :param grobid_config:
    ...
    with open(xml_fpath, 'r') as f:
        try:
            ...
            soup_html = BeautifulSoup(
                htmlmin.minify(
                    open(html_fpath, "r", encoding="utf-8").read().replace("\xa0", " "),
                    remove_all_empty_space=1,
                ),
                features="html.parser",
            )

            paper = convert_xml_to_s2orc(soup_xml, soup_html, file_id, year, log_file, temp_dir, grobid_config=grobid_config)
            return paper
